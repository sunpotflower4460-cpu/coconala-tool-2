...
- Tauri実アプリでのE2E確認(`tests/e2e-tauri/`でLinux上の初回設定・永続化・発行/変換/スナップショット・バックアップ復元・CSV取り込みを自動化済み。macOS/Windowsでの手動確認は残る)
- UI/UXの仕上げ(ホーム導線・左ナビのカテゴリ化・確認ダイアログ・AI結果の色以外の表示等は対応済み。
  ダークモードの実配色切り替えは未実装、実機での見た目の最終確認は未実施)
- 上書きアップデートでの既存DB保持確認
